...
export function AddTodo ({data, title, editing, cancel, save, deleteTodo, id, editTodo, doneTodo, isDone}) {
  const [titles, setTitles] = useState(title) //? sauvegarde du titre lorsqu'il est modifié
  const [datas, setDatas] = useState(data)//? sauvegarde de la description lorsqu'elle est modifié
  ...
  //? handle de toute les fonction pour permettre de les appeler correctement
  const handleDelete = () => {
    deleteTodo(id);
  }
          ...
          <div className="card-body">
            <div className="d-flex justify-content-between">
              <div className="d-flex ms-4 justify-content-center flex-column align-items-center pe-5 border-end">
                {!editing && <h5 className={isDone ? "card-title text-success" : 'card-title'} value={titles} >{title}</h5>}
                {editing && <input type="text" className="form-control" defaultValue={title} aria-label="Titre" onChange={(e) => setTitles(e.target.value)}></input>}
              </div>
              <div className="d-flex flex-column justify-content-center p-3">
                {editing && <textarea className="form-control w-100" defaultValue={data} aria-label="Description" onChange={(e) => setDatas(e.target.value)}></textarea>}
              ...
              {editing && <div className="form-check mx-3 d-flex flex-column justify-content-center align-items-center ps-5 border-start">
                <button onClick={handleSave} type="button" className="btn btn-primary">Enregistrer</button>
                <button onClick={handleCancel} type="button" className="btn btn-danger mt-2">Annuler</button>
              </div>
              }
